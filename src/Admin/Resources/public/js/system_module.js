var admin_system_module = new Class({
                ...
                if (!['core', 'admin', 'users'].contains(key)) {
                    if (item.activated) {
                        ...
                        new ka.Button(_('Activate')).addEvent('click', function () {
                            new Request.JSON({url: _pathAdmin + 'admin/system/module/manager/activate/', noCache: 1, onComplete: function () {
                                this.loadInstalled();
                                ka.loadSettings();
                                ka.loadMenu();
                            }.bind(this)}).get({name: key});
    ...
    loadLocal: function () {
        ...
        this.lc = new Request.JSON({url: _pathAdmin + 'admin/system/module/manager/local', noCache: 1, onComplete: function (res) {
            this.loader.hide();
    ...
    openSidebar: function (pBoxes) {
        ...
        pBoxes.each(function (opts) {
            ...
            this.oldGetbox = new Request.JSON({url: _pathAdmin + 'admin/system/module/managerGetBox/', noCache: 1, onComplete: function (res) {
                if (res) {
                    if (opts.render) {
