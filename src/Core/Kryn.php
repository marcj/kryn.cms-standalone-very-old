...
use Core\Exceptions\BundleConfigNotFoundException;
...
use Core\Exceptions\ModuleDirNotFoundException;
...
class Kryn extends Controller
{
    ...
    {
        if (!isset(Kryn::$config['languages']) && $pLang == 'en') return true; //default
        ...
        foreach ($plugins as $plugin) {
            ...
            if ($pluginDefinition && ($pluginRoutes = $pluginDefinition->getRoutes())) {
                foreach ($pluginRoutes as $route) {
                    ...
                    $routes->add($route->getId() ?: $routes->count()+1,
                        new Route(
