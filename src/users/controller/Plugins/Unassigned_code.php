<?php
    ...
    public function cleanMsgs($pMsg)
    {
    ...
    public function pluginMessageSystemInbox($pConf)
    {
        global $user;
    ...
    public function pluginMessageSystemOutbox($pConf)
    {
        global $user;
        //check if user is logged in
        if(!isset($user->user_id) || $user->user_id < 1)
    ...
    public function pluginMessageSystemNew($pConf)
    {
        global $user;
        //check if user is logged in
        if(!isset($user->user_id) || $user->user_id < 1)
        ...
        if (getArgv('sendNewMessage')) {
            ...
            } elseif (strpos($to, '@') !== false && strpos($to, '.') !== false) { // email
    ...
    public function pluginMessageSystemCountNew($pConf)
    {
        global $user;
        //check if user is logged in
    ...
    public static function pluginRegistration( $pConf )
    {
        ...
        if (getArgv('postdata') == 1) {
            ...
            if ($actKey != "") { // Activation key set, thus send email
    ...
    public static function pluginActivation( $pConf )
    {
        ...
        if($email == null || $actKey == null)
            ...
            if ($result === false) { // Email/key combo not found!
                $result['form'] = true;
                $result['failed'] = true;
            } else { // Email/key combo found, remove user activation key
                $id = $result['id'];
                $sql = "
