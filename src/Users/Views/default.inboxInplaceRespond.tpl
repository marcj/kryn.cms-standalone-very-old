        ...
        <form method="post" action="{$page|@realUrl}" id="msg-action-form">
            ...
            " id="one-message-pre-{$oneMessage.id}">
                        <td rowspan="2" class="user-message-system-one-message-td-action-select">
                            <input type="checkbox" name="one-message-action[]" value="{$oneMessage.id}"
                                   class="user-message-system-one-message-action-select">
                        </td>
                        <td class="one-message-left">
                            <div class="user-message-system-one-message-date">
                                {$oneMessage.send_tstamp|date_format:"%d.%m.%Y um %H:%I"}
                            </div>
                            <div id="openMessageBtn{$oneMessage.id}"
                                 onclick="openMessage($('openMessageBtn{$oneMessage.id}'), '{$oneMessage.id}')"
                                 class="user-message-system-one-message-subject one-message-subject-{$oneMessage.message_state}"
                                 title="[[show / hide this message]]">
                        ...
                        <td>
                            <div class="user-message-system-one-message-panel">
                                ...
                                <div class="panel-item">
                                    <a onclick="openRespondPanel({$oneMessage.id}); return false;"
                                       href="{$newMessagePage|@realUrl}?oldMessageRsn={$oneMessage.id}"
                                       class="user-message-system-one-message-respond">
                                        [[respond]]
                                    </a>
                                </div>
                                {if $newMessagePage}
                                    <div class="panel-item">
                                        <a href="{$newMessagePage|@realUrl}?oldMessageRsn={$oneMessage.id}&amp;type=fwd"
                                           class="user-message-system-one-message-forward">
                                            [[forward]]
                                        </a>
                                    </div>
                                {/if}
                                <div class="panel-item no-border">
                                    <a id="delMsgBtn{$oneMessage.id}"
                                       onclick="askDelete($('delMsgBtn{$oneMessage.id}')); return false;"
                                       href="{$page|@realUrl}?action_select=delete&amp;one-message-action[]={$oneMessage.id}"
                                       class="user-message-system-one-message-forward">
                                        [[delete]]
                                    </a>
                                </div>
              ...
              no-border">
                        <td colspan="2">
                            <div id="one-message-text-{$oneMessage.id}" style="display:none;">
                                ...
                                <b>[[Respond]]</b>
                                <textarea id="one-message-response-textarea-{$oneMessage.id}"
                                          class="user-message-system-one-message-textarea"></textarea>

                                <div id="sendMsgBtn{$oneMessage.id}" class="user-message-system-one-message-send-btn"
                                     onclick="sendAjReMessage({$oneMessage.id}, $('sendMsgBtn{$oneMessage.id}'))">
                                    [[Send]]
                                </div>
                                <div class="user-message-system-one-message-send-btn"
            ...
            {if $pages > 1 }
                <div class="user-message-system-ibox-pages">[[Page]]:
                    {section name=messagePage start=1 loop=$pages+1}
                        ...
                        {else}
                            <a href="{$page|@realUrl}/{$smarty.section.messagePage.index}/"
                               class="user-message-system-ibox-one-page">{$smarty.section.messagePage.index}</a>
                        {/if}
                    {/section}
                </div>
            {/if}
