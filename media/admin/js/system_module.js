var admin_system_module = new Class({
        ...
        this.llir = new Request.JSON({url: _path + 'admin/system/module/loadInstalled', noCache: 1, onComplete: function (res) {
            ...
            Object.each(res, function (item, key) {
                ...
                if (item.version != item.serverVersion && item.serverVersion != '') {
                    ...
                    new ka.Button(item.serverCompare == '>' ? _('Downgrade') : _('Update')).addEvent('click', function () {
                        ka.wm.open('admin/system/module/view/', {name: key, type: 0, updateNow: 1});
                    }.bind(this)).inject(actions);
                }

                var value = [title, '<img title="' + _title + '" src="' + _path + PATH_MEDIA + '/admin/images/icons/' + icon + '.png" />', item.version, (item.serverVersion) ? item.serverVersion : _('Local'), actionsStatus, actions];
    ...
    renderLocal: function (pMods) {
        ...
        Object.each(pMods, function (mod, key) {
            ...
            var value = [title + ' <span style="color: gray;">(' + key + ')</span>', '<img title="' + _title + '" src="' + _path + PATH_MEDIA + '/admin/images/icons/' + icon + '.png" />', item.version, (!item.owner || item.owner == "") ? _('Local') : item.serverVersion, bActions, actions];
    ...
    doSearch: function () {
        ...
        this.mainPane.set('html', '<center><img src="' + _path + PATH_MEDIA + '/admin/images/loading.gif" /></center>');
    ...
    _createListItem: function (pItem) {
        ...
        new Element('div', {
            html: pItem.desc + '<br /><br/>Extensioncode: #' + pItem.name + '<br />' + 'Version: ' + pItem.version + '<br />' + 'Downloads: ' + pItem.downloads
        }).inject(desc);
