ka.layoutContent = new Class({
    ...
    renderToolbar: function () {
        ...
        }.bind(this)).inject(this.toolbarTitleContainer);

        this.iTitle.store('empty', true);
        this.iTitleSetBlankText();

        this.imgAccess = new Element('img', {
            src: _path + PATH_MEDIA + '/admin/images/admin-pages-viewType-versioning.png',
            'class': 'ka-layoutContent-toolbar-access',
            title: _('Access settings')
        }).addEvent('click', this.openAccessDialog.bind(this)).inject(this.toolbarTitleContainer);

        this.sType = new ka.Select().addEvent('change', this.changeType.bind(this)).inject(this.toolbarTitleContainerSelects);
        ...
        this.sTemplate = new ka.Select().addEvent('change', function () {
            this.content.template = this.sTemplate.getValue();
            this.setDivContent();
        }.bind(this)).inject(this.toolbarTitleContainerSelects);
    ...
    openAccessDialog: function () {
        ...
        new ka.Button(_('Cancel')).addEvent('click', dialog.close.bind(dialog)).inject(bottom);
    ...
    setBubbleContent: function () {
        ...
        this.bubbleBoxContent.set('html', '<div style="font-weight: bold;">' + title + '</div>' + _('Template file') + ': ' + this.getTemplateTitle(this.content.template) + '<br />' + '');

    },

    renderTitleActions: function () {
        var p = _path + PATH_MEDIA + '/admin/images/icons/';
    ...
    type2Navi: function () {
        ...
        var templateNavi = new ka.field({label: _('Navigation template'), type: 'select', small: 1}).inject(this.body);
    ...
    type2Pointer: function () {
        ...
        var field = new ka.field({label: _('Choose deposit'), type: 'pageChooser', empty: false, small: true}).inject(this.body);
    ...
    setHide: function (pHide) {
        ...
        if (this.content.hide == 0) {
            this.hideImg.set('src', _path + PATH_MEDIA + '/admin/images/icons/lightbulb.png');
        } else {
            this.hideImg.set('src', _path + PATH_MEDIA + '/admin/images/icons/lightbulb_off.png');
        }
    },
    ...
    setDivNavigation: function (pContainer) {
        ...
        this.bodyNavigationRequest = new Request.JSON({url: _path + 'admin/backend/navigationPreview/', noCache: 1, onComplete: function (html) {
            ...
            new Element('img', {
                src: _path + PATH_MEDIA + '/admin/images/icons/bullet_go.png',
                title: _('Open target')
            }).addEvent('click', function () {
                this.w.kpage.loadPage(this.content.content, true);
            }.bind(this)).inject(div, 'top');
