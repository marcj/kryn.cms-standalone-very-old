ka.LayoutBox = new Class({
    initialize: function (pElement, pOptions, pClassObj) {//pTitle, pWin, pContentCss, pDefaultTemplate, pPageInst, pAllOptions ){
        ...
        }).addEvent('click', this.add.bind(this)).inject(this.title);
    ...
    addButtonBar: function () {
        ...
        }).addEvent('click', this.pasteAfter.bind(this)).inject(this.title);

        new Element('img', {
            src: p + 'page_copy.png',
            title: _('Copy all'),
            'class': 'ka-layoutBox-iconBar-icon'
        }).addEvent('click', this.copyAll.bind(this)).inject(this.title);
