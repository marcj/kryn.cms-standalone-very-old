...
class krynSearch extends baseModule{
    ...
    public static function getLinksInContent($pContent) {
        ...
        foreach($matches as $value) {
           ...
           if( !isset($arInserted[kryn::$domain['rsn'].'_'.$value[1]]) && !isset($arInserted[kryn::$domain['rsn'].'_'.$value[1]]) && strlen($value[1]) > 0 ){
    ...
    public static function disposePageForIndex($pUrl, $pTitle, $pDomainRsn, $pPageRsn='0') {
        ...
        $url = esc($pUrl);
        $row = dbExfetch("SELECT rsn FROM %pfx%system_search WHERE url = '$url' AND domain_rsn = ".$pDomainRsn+0, 1);
