...
/**
 * Base class that represents a row from the 'kryn_system_group' table.
 *
 * 
 *
...
abstract class BaseGroup extends BaseObject 
{
    ...
    public function setClose($v)
    {
        ...
        if ($this->close !== $v) {
            $this->close = $v;
            $this->modifiedColumns[] = GroupPeer::CLOSE;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->close = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if (null === $this->id) {
            try {				
				$stmt = $con->query("SELECT nextval('kryn_system_group_id_seq')");
				$row = $stmt->fetch(PDO::FETCH_NUM);
				$this->id = $row[0];
            } catch (Exception $e) {
        ...
        if ($this->isColumnModified(GroupPeer::CLOSE)) {
        ...
        $sql = sprintf(
            'INSERT INTO kryn_system_group (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'ID':
                        ...
                        break;
                    case 'CLOSE':
						$stmt->bindValue($identifier, $this->close, PDO::PARAM_INT);
                        break;
                    case 'NAME':
    ...
    public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) $this->setClose($arr[$keys[1]]);
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(GroupPeer::CLOSE)) $criteria->add(GroupPeer::CLOSE, $this->close);
