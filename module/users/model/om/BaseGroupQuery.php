...
/**
 * Base class that represents a query for the 'kryn_system_group' table.
 *
 * 
 *
 * @method     GroupQuery orderById($order = Criteria::ASC) Order by the id column
 * @method     GroupQuery orderByClose($order = Criteria::ASC) Order by the close column
 ...
 * @method     GroupQuery groupByClose() Group by the close column
 ...
 * @method     Group findOneByClose(int $close) Return the first Group filtered by the close column
 ...
 * @method     array findByClose(int $close) Return Group objects filtered by the close column
...
abstract class BaseGroupQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT ID, CLOSE, NAME, DESCRIPTION FROM kryn_system_group WHERE ID = :p0';
        try {
    ...
    public function filterByClose($close = null, $comparison = null)
    {
        if (is_array($close)) {
            $useMinMax = false;
            if (isset($close['min'])) {
                $this->addUsingAlias(GroupPeer::CLOSE, $close['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($close['max'])) {
                $this->addUsingAlias(GroupPeer::CLOSE, $close['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(GroupPeer::CLOSE, $close, $comparison);
    }
