...
/**
 * Base class that represents a query for the 'kryn_system_user_group' table.
 *
 * 
 *
...
abstract class BaseUserGroupQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT GROUP_ID, USER_ID FROM kryn_system_user_group WHERE GROUP_ID = :p0 AND USER_ID = :p1';
        try {
    ...
    {
        $this->addUsingAlias(UserGroupPeer::GROUP_ID, $key[0], Criteria::EQUAL);
        $this->addUsingAlias(UserGroupPeer::USER_ID, $key[1], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            $cton0 = $this->getNewCriterion(UserGroupPeer::GROUP_ID, $key[0], Criteria::EQUAL);
            $cton1 = $this->getNewCriterion(UserGroupPeer::USER_ID, $key[1], Criteria::EQUAL);
            $cton0->addAnd($cton1);
            $this->addOr($cton0);
        }
    ...
    {
        if ($userGroup) {
            $this->addCond('pruneCond0', $this->getAliasedColName(UserGroupPeer::GROUP_ID), $userGroup->getGroupId(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond1', $this->getAliasedColName(UserGroupPeer::USER_ID), $userGroup->getUserId(), Criteria::NOT_EQUAL);
